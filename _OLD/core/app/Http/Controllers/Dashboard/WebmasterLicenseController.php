...
class WebmasterLicenseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->action == "check_license" && $request->purchase_code != "") {
            ...
            if ($this->system_update()) {
                return ["status" => "updated", "msg" => __('backend.updateCompleted'), "version" => "10.0.0"];
            }
    ...
    private function check_updates()
    {
        try {
            $WebmasterSetting = WebmasterSetting::find(1);
            $response = $this->api_call("check_update", decrypt(@$WebmasterSetting->purchase_code), @$WebmasterSetting->version);
            if (@$response['status'] == "success") {
                if (@$WebmasterSetting->version == @$response['version']) {
                    return ["status" => "upto_date", "msg" => __('backend.websiteUptoDate'), "version" => @$response['version']];
