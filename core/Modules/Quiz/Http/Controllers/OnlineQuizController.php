...
//                $query->whereIn('user_id', $ids);
    ...
    public function enrolledStudent($id, Request $request)
    {
        try {
            ...
            if (empty($quiz_type)) {
                $queryQuizTest = QuizTest::where('quiz_id', $quiz->id)->with('details', 'quiz', 'user', 'course');
                if (isModuleActive('AdvanceQuiz')) {
                    $queryQuizTest->whereHas('course', function ($q) {
                        $q->where('advance_test', 1);
                    });
                }
                $quizTests = $queryQuizTest->get();
