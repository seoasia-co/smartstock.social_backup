    ...
    public function SMAI_Check_Universal_UserPlans($user_id, $database, $platform)
    {
        ...
        if ($user != NULL) {
            ...
            if ($database == 'main_db' && ($platform == 'MainCoIn' || $platform == 'maincoin' || $platform == 'main_coin')) {
                ...
                $user_main_subscription = SubscriptionMain::where('user_id', $user_id)
                ...
                if ($user_main_subscription) {
                    $user_main_plan = $user_main_subscription->plan_id;
                    $endsAt = Carbon::parse($user_main_subscription->ends_at);
                    $trialEndsAt = Carbon::parse($user_main_subscription->trial_ends_at);

                    if ($endsAt->greaterThan($trialEndsAt)) {
                        // ends_at is later than trial_ends_at
                        // Handle accordingly
                        $user_plan_expire = $user_main_subscription->ends_at;
                    } else {
                ...
                $user_main->plan_expire_date = $user_plan_expire;
                $user_main->save();
                ...
                if ($user_plan_expire == NULL || $user_main_plan == NULL) {
                    $user_main_subsccription = SubscriptionMain::where('user_id', $user_id)
                        ->where(function ($query) {
                            $query->where('stripe_status', 'active')
                                ->orWhere('stripe_status', 'trialing');
                        })
